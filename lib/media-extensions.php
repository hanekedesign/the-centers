...
if ($attachments) {
    foreach ($attachments as $post) {
      ...
      if ($state == null || $subbox == "@new@") continue;
      if (!isset($map[$state])) $map[$state] = array();
      if (!in_array($subbox,$map[$state])) $map[$state][] = $subbox;
    }
}
...
function render_parameters_box() {
  ...
  <script>
      ...
      _$('#media-ext-folder').change(function() {
        ...
        var map = statedatamap[_$('#media-ext-folder').val()];
        //alert(statedatamap[_$('#media-ext-folder').val()]);
